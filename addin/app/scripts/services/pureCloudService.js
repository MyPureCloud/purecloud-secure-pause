...
angular.module('pureCloudService', ['ab-base64', 'chromeStorage'])
  .service('pureCloudService', function ($http, $log, $httpParamSerializerJQLike, $q, base64, chromeStorage) {
    ...
    function GetOptions() {
      ...
      //_clientId = '06887ffe-f613-4f97-bf22-6b13faade8fd'; -- PRD
      _clientId = '4d1bd767-9753-4f08-bfca-093523145d10'; // Test
      _environment = 'mypurecloud.ie';
      _callbackURI = 'https://ipepiaoogopplecfilkpjkibljjcmhee.chromiumapp.org/index.html';
        ...
        sendRestRequest('channel.create', 'POST', apipath + '?' + $httpParamSerializerJQLike(queryParameters), requestBody).then(function success(response) {
          ...
          _webSocket.onmessage = function (message) {
            ...
            if (data.topicName !== 'channel.metadata') {
              var sObject = data.eventBody.participants[1];
              if (sObject.calls === undefined) {
                return;
              };
              ...
              */
              // Find last Agent -- New Version

              for (var obj in data.eventBody.participants) {
                if (data.eventBody.participants[obj].purpose.toLowerCase() == 'agent') {
                  if (data.eventBody.participants[obj].calls != null && data.eventBody.participants[obj].calls[0].state.toLowerCase() == 'connected') {

                    _CallID = data.eventBody.id; // Save CallID of this interaction
                    console.log('Found active callID: ' + _CallID);
                  } else if (data.eventBody.participants[obj].calls != null && data.eventBody.participants[obj].calls[0].state.toLowerCase() == 'disconnected') {
                    _CallID = "";
                    _bSecurePause = false;
                  }
                }
    ...
    function sendRestRequest(requestName, method, path, body) {
      ...
      var options = {
        method: method,
        url: 'https://' + "api.mypurecloud.ie" + path,
        headers: {
      ...
      console.log('Token value: ' + _accessToken);
